...
import org.apache.flink.streaming.api.datastream.{ConnectedStreams as JavaCStreams, DataStream as JavaStream}
...
final case class ConnectedStreams[A, B](streams: JavaCStreams[A, B]) {
  ...
  def biMapWithState[O, S](
    ...
    val biMapper: RichCoMapFunction[A, B, O] with ResultTypeQueryable[O] =
      new RichCoMapFunction[A, B, O] with ResultTypeQueryable[O] {
